...
void Rmp440LE::UpdateOdometry()  
{
  ...
  double wheelbase = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::FRAM_WHEEL_TRACK_WIDTH_M));
  double secs = ros::Time::now().toSec();
  ros::Time then = ros::Time::now();

  if (m_OdometryInitialized)
  {
    double leftwheel = ((leftFrontDisplacement - m_WheelsDisplacement.m_LeftFront) + (leftRearDisplacement - m_WheelsDisplacement.m_LeftRear)) / 2.0;
    // std::cout << "leftwheel" << leftwheel <<std::endl;
    double rightwheel = ((rightFrontDisplacement - m_WheelsDisplacement.m_RightFront) + (rightRearDisplacement - m_WheelsDisplacement.m_RightRear)) / 2.0;
    // std::cout << "righwheel" << rightwheel <<std::endl;
    double leftwheelvel = ((leftfrontvel+leftrearvel)/2.0);
    // std::cout << "leftvel" << leftwheelvel <<std::endl;
    double rightwheelvel = ((rightfrontvel+rightrearvel)/2.0);
    // std::cout << "rightvel" << rightwheelvel<<std::endl;
    double timeleft = (leftwheel/leftwheelvel);
    // std::cout << "timeleft" << timeleft <<std::endl;
    double timeright = (rightwheel/rightwheelvel);
    // std::cout << "timeright" << timeright <<std::endl;
    double time = (timeright+timeleft)/2.0;
    double theta = (rightwheelvel-leftwheelvel)*time/1.08616489885305;
    // std::cout << theta <<std::endl;
    double d = (leftwheel+rightwheel)/2.0;
    th_=th_+theta;
    std::cout << th_*180/M_PI<<std::endl;
    double vel = (leftwheelvel + rightwheelvel)/2.0;
    double x =cos(theta)*d;
    double y=-sin(theta)*d;
    x_=x_+(cos(th_)*x-sin(th_)*y);
    y_=y_+(sin(th_)*x+cos(th_)*y);
  //   double leftwheel= (leftFrontDisplacement+leftRearDisplacement)/2.0;
  //   double rightwheel= (rightFrontDisplacement+rightRearDisplacement)/2.0;
  //   double theta = (rightwheel-leftwheel)/wheelbase;
  //   std::cout << theta*180/M_PI <<std::endl;
    ...
    // // std::cout << "leftfront disp." << leftfrontvel <<std::endl;
    // // std::cout << "leftrear disp." << leftrearvel <<std::endl;
    // // std::cout << "rightfront disp." << rightfrontvel <<std::endl;
    // // std::cout << "rightrear disp." << rightrearvel <<std::endl;
    // // std::cout << "leftfront wheel." << m_WheelsDisplacement.m_LeftFront <<std::endl;
    // double leftwheel= (leftFrontDisplacement+leftRearDisplacement)/2.0;
    // double rightwheel= (rightFrontDisplacement+rightRearDisplacement)/2.0;
    // // std::cout << "leftdisp." << leftwheel <<std::endl;
    // // std::cout << "right disp." << rightwheel <<std::endl;
