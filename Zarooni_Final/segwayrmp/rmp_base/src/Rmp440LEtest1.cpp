...
void Rmp440LE::InitializeMessages()
{
  ...
  m_NodeHandle.param("odometry_frame", odometryFrame, std::string("/rmp440le/odomm"));
...
void Rmp440LE::UpdateOdometry()  
{
  ...
  double sec;
  double leftPositionLast_ = 0.0;
  double rightPositionLast_ = 0.0;

      // std::cout << elapsed << std::endl;
      // std::cout << sec << std::endl;
  if (m_OdometryInitialized)
  {
    // double vl = (leftfrontvel+leftrearvel)/2.0;
    // double vr = (rightfrontvel+rightrearvel)/2.0;
    // double deltaLinearDisplacement = linearDisplacement - m_WheelsDisplacement.m_Linear;

    // std::cout << "linear disp." << linearDisplacement <<std::endl;
    // std::cout <<"wheel disp." << m_WheelsDisplacement.m_Linear <<std::endl;
    // std::cout << "leftfront disp." << leftFrontDisplacement <<std::endl;
    // std::cout << "leftrear disp." << leftRearDisplacement <<std::endl;
    // std::cout << "rightfront disp." << rightFrontDisplacement <<std::endl;
    // std::cout << "rightrear disp." << rightRearDisplacement <<std::endl;
    // std::cout << "leftfront wheel." << m_WheelsDisplacement.m_LeftFront <<std::endl;
    ...
    // std::cout << deltaLeftSideDisplacement << std::endl;
    // ros::Duration elapsed = ros::Time::now() - now;
    // sec = elapsed.toSec();
    // // std::cout << elapsed << std::endl;
    // double velocity_left = deltaLeftSideDisplacement/sec;
    // std::cout << velocity_left<< std::endl;
    ...
    // std::cout << deltaRightSideDisplacement << std::endl;
    // ros::Duration elapsed = ros::Time::now() - now;
    // sec = elapsed.toSec();
    // std::cout << elapsed << std::endl;
    // double velocity_right = deltaRightSideDisplacement/sec;
    // std::cout << velocity_right<< std::endl;
    ...
    // double d = (deltaLeftSideDisplacement+deltaRightSideDisplacement)/2.0; //linear velcoity
    // std::cout << wheelTrackWidth << std::endl;
    // double th = (deltaRightSideDisplacement-deltaLeftSideDisplacement)/wheelTrackWidth; //angular velocity
    // std::cout << th << std::endl;
    // double x = cos(th)*d;
    // double y = -sin(th)*d;
    double leftPosition = (leftFrontDisplacement+leftRearDisplacement)/2.0;
    double rightPosition = (rightFrontDisplacement+rightRearDisplacement)/2.0;
    double x = (leftPosition+rightPosition)/2.0;
    double wheelTrackWidth = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::FRAM_WHEEL_TRACK_WIDTH_M));
    // std:: cout << wheelTrackWidth << std::endl;
    double th = (rightPosition-leftPosition)/1.0640519609497;
    // double th = (vr-vl)/wheelTrackWidth;

    double leftDistance = (leftPosition - leftPositionLast_); 
    double rightDistance = (rightPosition - rightPositionLast_);
    double delta_distance = (leftDistance + rightDistance) / 2.0; 
    double delta_theta = (rightDistance - leftDistance) / 0.84201;
    ...
    th_=th_+th;
    std::cout << th_*180/M_PI << std::endl;
    // // std::cout << "wheel" << wheelTrackWidth <<std::endl;
    // double deltaHeading = (deltaRightSideDisplacement - deltaLeftSideDisplacement) / wheelTrackWidth;
    // double deltaHeadingyaw = (deltaRightSideDisplacement*2.0 - deltaLeftSideDisplacement*2.0) / wheelTrackWidth;

    // double previousHeading = tf::getYaw(m_OdometryMsg.pose.pose.orientation);
    // std::cout << "prev. heading: " << (previousHeading*(180/M_PI)) <<std::endl;
    ...
    geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromRollPitchYaw(0.0,0.0,th_);
